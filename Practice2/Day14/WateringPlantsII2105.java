...
public class WateringPlantsII2105 {
    public static int minimumRefill(int[] plants, int capacityA, int capacityB) {
        int i = 0, j = plants.length - 1, a = 0;
        while (i < plants.length && j >= i) {
            if (a >= plants[i])
                a = plants[i] - a;
            if (plants[i] <= capacityA) {
                a = plants[i] - capacityA;
                i++;
            }
            if (plants[j] - capacityB <= capacityB && plants[j] < capacityB) {
                j++;
            }
        }
        return 0;
 ...
 * in a row and are labeled from 0 to n - 1 from left to right where the ith
 ...
 * amount of water the ith plant needs, and two integers capacityA and capacityB
